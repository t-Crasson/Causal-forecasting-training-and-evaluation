...
class MIMIC3RealDataset(Dataset):
    ...
    def __init__(self,
        ...
        subject_ids = treatments.index.get_level_values("subject_id").values.reshape((len(user_sizes), max(user_sizes), -1)).astype(float)
...
class MIMIC3RealDatasetRDD(Dataset):
    """
    Pytorch-style real-world MIMIC-III dataset
    """
    def __init__(self,
    ...
    def process_sequential_multi(self, projection_horizon):
        ...
        return self.data
class MIMIC3RealDatasetCollectionRDD(RealDatasetCollection):
    """
    Dataset collection (train_f, val_f, test_f)
    """
    def __init__(self,
        ...
        Args:
            ...
            autoregressive:
        """
        super(MIMIC3RealDatasetCollectionRDD, self).__init__()
        self.seed = seed
        treatments, outcomes, vitals, static_features, outcomes_unscaled, scaling_params = \
            load_mimic3_data_processed(ROOT_PATH + '/' + path, min_seq_length=min_seq_length, max_seq_length=max_seq_length,
                                       max_number=max_number, data_seed=seed, **kwargs)
        ...
        self.train_f = MIMIC3RealDatasetRDD(treatments_train, outcomes_train, vitals_train, static_features_train,
                                         outcomes_unscaled_train, scaling_params, 'train')
        if split['val'] > 0.0:
            self.val_f = MIMIC3RealDatasetRDD(treatments_val, outcomes_val, vitals_val, static_features_val, outcomes_unscaled_val,
                                           scaling_params, 'val')
        self.test_f = MIMIC3RealDatasetRDD(treatments_test, outcomes_test, vitals_test, static_features_test, outcomes_unscaled_test,
                                        scaling_params, 'test')
